...
public class ItemSpawnEgg  extends ItemMonsterPlacer
{
    ...
    protected boolean doneDisplayName = false;
    ...
    public ItemSpawnEgg(String parEntityToSpawnName, int parPrimaryColor, int parSecondaryColor)
    {
        ...
        // DEBUG
        //System.out.println("[DM] Spawn egg constructor for " + entityToSpawnName + " successful.");
    }
    ...
    public Entity spawnEntity(World parWorld, double parX, double parY, double parZ)
    {
    	...
    	{
            entityToSpawnNameFull = DungeonMobs.MODID + "." + entityToSpawnName;
    ...
    {
    	if(!this.doneDisplayName)
    	{
    		String s = entityToSpawnName;
	    	
	    	String[] foo = s.split("DM");
	    	
	    	String[] bar = foo[1].split("(?=[A-Z])");
	    	...
	    	{
	    		if(ret.charAt(i - 1) == ' ' && ret.charAt(i) == ' ')
	    		{
	    			String z = ret.substring(i);
	    			String y = ret.substring(0, i - 1);
	    			ret = y + z;
